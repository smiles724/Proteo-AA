...
上一节的探针仍是**条件生成**（有靶点），binder 链的几何失败和「模型还得重建靶点」
混在一起。`scripts/evaluation/probe_monomer_backbone_geometry.py` 去掉靶点：真实
Protenix 单体行、全残基设计位、从高斯噪声起、400 步 `pxdesign_native`
（12 个样本，80–200 残基，jobs 114668–114670）。

| checkpoint | 单体 CA–CA 中位 / 坏键 / Rg | 复合体 binder 坏键 |
| --- | --- | --- |
| PXDesign official | 3.81 / **8.0%** / 12.9 | **0.0%** |
| Stage II 52500 | 3.66 / **60.1%** / 13.0 | 98.1% |
| Stage III 111408 s6000 | 3.92 / **60.5%** / 17.0 | 85.2% |

官方在这个探针下 8.0%，说明探针本身可信。结论是**两个失效叠加**，不是二选一：

1. **基础主链几何本来就不合格** —— 60% 坏键 vs 官方 8%。
2. **复合体再叠一层退化** —— 同一个 checkpoint 从 60% 掉到 85–98%，Stage II 的
   Rg 更是从 13.0 塌到 6.8。这部分确实来自靶点重建。

还有一条：**Stage III 相对 Stage II 在单体上没有改善**（60.1% → 60.5%），
在复合体上却明显改善（98.1% → 85.2%）。和它 75% PINDER 复合体的数据配比一致 ——
那 6650 步学的是复合体/条件化，没有回头补主链几何。

## 下一步

**注意：这里原先写的是「缺的是无条件主链生成的训练阶段，不是靶点条件化」，
那句话只对了一半**，上面的单体数据出来后已更正为「两个失效叠加」。
