#!/usr/bin/env bash
#SBATCH --job-name=sc-repair-v2-extend
...
#SBATCH --output=logs/training/sc-repair-v2-extend-%j.out
#SBATCH --error=logs/training/sc-repair-v2-extend-%j.err
set -euo pipefail
...
read -r ARM CHECKPOINT < <(
  "$PYTHON_BIN" -c 'import json,sys; s=json.load(open(sys.argv[1])); assert s["passed"]; print(s["selected"]["arm"],s["selected"]["checkpoint"])' \
    "$RUN_ROOT/screen.json"
)
...
exec "$PYTHON_BIN" scripts/training/train_sc_adaptation.py \
  --resume-checkpoint "$CHECKPOINT" --output-dir "$RUN_ROOT/arm_${ARM}_extended" \
  --max-steps 5000
