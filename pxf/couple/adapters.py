...
class NoiseEmbedding(nn.Module):
    ...
    def forward(self, sigma):
        ...
        frequencies = torch.exp(
            ...
            * torch.arange(half, dtype=torch.float32, device=value.device) / half)
...
class ResidualAdapter(nn.Module):
    ...
    def forward(self, source, sigma):
        if source.shape[-1] != self.d_in:
            raise ValueError(f"Adapter expects last dim {self.d_in}, got {source.shape[-1]}")
        ...
        if embedded.shape[0] != normalized.shape[0]:
            raise ValueError(f"Got {embedded.shape[0]} sigma values for "
                             f"{normalized.shape[0]} examples")
        # Broadcast the per-example noise embedding across residues.
        shape = (normalized.shape[0],) + (1,) * (normalized.dim() - 2) + (embedded.shape[-1],)
        embedded = embedded.reshape(shape).expand(*normalized.shape[:-1], embedded.shape[-1])
    ...
    def is_identity(self):
        ...
        return (bool(torch.all(self.project_out.weight == 0))
                and bool(torch.all(self.project_out.bias == 0)))
...
class CouplingAdapters(nn.Module):
    ...
    def __init__(self, d_backbone, d_fampnn, *, d_hidden=256, d_noise=64,
                 enable_bb_to_sc=True, enable_sc_to_bb=True):
        super().__init__()
        self.d_backbone, self.d_fampnn = int(d_backbone), int(d_fampnn)
        self.bb_to_sc = ResidualAdapter(d_backbone, d_fampnn, d_hidden=d_hidden,
                                        d_noise=d_noise)
        self.sc_to_bb = ResidualAdapter(d_fampnn, d_backbone, d_hidden=d_hidden,
                                        d_noise=d_noise)
    ...
    def set_phase(self, phase):
        ...
        self.phase = phase
        return dict(phase=phase, bb_to_sc=train_bs, sc_to_bb=train_sb,
    ...
    def identity(self):
        return dict(d_backbone=self.d_backbone, d_fampnn=self.d_fampnn,
