...
def test_packed_sidechains_reproduce_the_withheld_native(packer, native):
    ...
    compare = (native["atom_mask"][..., SC].bool() & ~mask[..., SC].bool()).to(
        packer.device
    )
    assert int(compare.sum()) > 300, "the target should have side chains to compare against"
    delta = (out["coords_af2"][..., SC, :] - native["x"][..., SC, :].to(packer.device))[
        compare
    ]
...
def test_batching_packs_every_sample(packer, native):
    coords, mask = _backbone_only(native)
    out = packer(
        coords_af2=coords.expand(3, -1, -1, -1),
        atom_mask=mask.expand(3, -1, -1),
        aatype=native["aatype"],
        residue_index=native["residue_index"],
        chain_index=native["chain_index"],
    )
...
def test_packing_is_a_sampler_so_repeats_differ_but_stay_close(packer, native):
    ...
    coords, mask = _backbone_only(native)
    out = packer(
        coords_af2=coords.expand(2, -1, -1, -1),
        atom_mask=mask.expand(2, -1, -1),
        aatype=native["aatype"],
        residue_index=native["residue_index"],
        chain_index=native["chain_index"],
    )
    ...
    assert spread < 2.0, (
        f"same backbone and sequence should give similar packings, got {spread:.3f} A"
    )
...
def test_a_single_structure_may_omit_the_batch_axis(packer, native):
    ...
    out = packer(
        coords_af2=coords[0],
        aatype=native["aatype"][0],
        atom_mask=mask[0],
        residue_index=native["residue_index"][0],
        chain_index=native["chain_index"][0],
    )
