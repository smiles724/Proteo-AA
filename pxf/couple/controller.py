...
class BackboneDenoiser(Protocol):
    ...
    def __call__(self, x_noisy: torch.Tensor, sigma: torch.Tensor, *,
                 feedback: Optional[torch.Tensor] = None) -> tuple: ...
...
class CycleOutput:
    ...
    h_base: Optional[torch.Tensor] = None
    delta_h: Optional[torch.Tensor] = None
    h_cond: Optional[torch.Tensor] = None
    sidechains: Optional[torch.Tensor] = None  # [B, L, 33, 3] global
    h_packed: Optional[torch.Tensor] = None
    delta_a: Optional[torch.Tensor] = None
    bb1_flat: Optional[torch.Tensor] = None
...
class CoupledDenoiser:
    ...
    def __init__(self, backbone: BackboneDenoiser, fampnn, adapters, *,
                 converter=None, phase="joint", pack_steps=None,
    ...
    def set_phase(self, phase):
        ...
        record = self.adapters.set_phase(phase) if hasattr(self.adapters, "set_phase") else {}
        return dict(phase=phase, policy=vars(self.policy), adapters=record)

    # ---- the cycle -------------------------------------------------------

    def forward(self, topology, x_noisy, sigma, aatype, *, seq_mask=None,
                run_feedback=None, sidechain_context=None):
        """One cycle. ``run_feedback`` defaults to whether SC->BB is enabled."""
        converter = self.converter
        # --- backbone proposal ---
        bb0_flat, a_token = self.backbone(x_noisy, sigma)
        if a_token is None:
            raise ValueError("The backbone denoiser returned no token features; "
                             "coupling needs a_token (see pxf.couple.pxdesign_iface)")

        inputs = converter.px_backbone_to_fampnn(
            bb0_flat, topology.atom_names, topology.atom_to_token_idx,
            topology.num_tokens, res_names=topology.res_names,
            residue_index=topology.residue_index, chain_index=topology.chain_index,
            aatype=aatype)
        if seq_mask is not None:
            inputs = replace(inputs, seq_mask=converter.px_residue_mask_to_fampnn(
                seq_mask, topology.num_tokens, batch=inputs.batch))

        out = CycleOutput(bb0_flat=bb0_flat, bb0_dense=inputs.coords_af2,
                          a_token=a_token, aux=dict(inputs=inputs))

        # --- BB -> SC ---
        _, h_base, features = iface.encode(
            self.fampnn, inputs.coords_af2, inputs.aatype,
            seq_mask=inputs.seq_mask, missing_atom_mask=inputs.missing_atom_mask,
            residue_index=inputs.residue_index, chain_index=inputs.chain_index)
        out.h_base = h_base
        ...
        sidechains, pack_aux = iface.pack_from_features(
            self.fampnn, iface.with_residual(features, out.delta_h), inputs.aatype,
            seq_mask=inputs.seq_mask, residue_index=inputs.residue_index,
            chain_index=inputs.chain_index, num_steps=self.pack_steps)
        ...
        _, h_packed, _ = iface.encode(
            self.fampnn, inputs.coords_af2, inputs.aatype, sidechains=visible,
            seq_mask=inputs.seq_mask, missing_atom_mask=inputs.missing_atom_mask,
            residue_index=inputs.residue_index, chain_index=inputs.chain_index)
    ...
    def _match(delta, reference):
        ...
        if delta.shape[:2] != reference.shape[:2]:
            raise ValueError(f"adapter output {tuple(delta.shape)} does not match "
                             f"target latent {tuple(reference.shape)}")
        return delta

    def identity(self):
        return dict(phase=self.phase, policy=vars(self.policy),
