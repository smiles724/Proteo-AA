...
The HAI binder launcher replaces the base launcher's 100-step smoke defaults with a measured scale: IV-A, one training revision, crop 384, accumulation 8, gradient clip 1.0, and checkpoints every 1000 steps and evaluation every 2000 steps in a 23:50 slot. Crop 384 is affordable in IV-A only because the frozen backbone, packer and feedback modules retain no autograd graph; **IV-B/IV-C open the packer and the atom-attention decoder and have not been memory-proven at this crop.**

Explicit CLI options take precedence over the HAI wrapper's environment-derived defaults, including accumulation, validation intervals and worker counts. A leading `--dry-run` remains supported.

`evaluate()` runs *before* `save_checkpoint()` in the training loop, so a failure in the new binder/monomer validation path at the first eval step discards everything before it with no checkpoint written. That is not hypothetical: the named Stage IV loaders stamp a string `source` on every per-protein row, the per-protein log line formatted every field with `:.4g`, and `f"{'binder_pinder':.4g}"` raises `ValueError: Unknown format code 'g' for object of type 'str'`. Fixed by emitting `val_source=` explicitly, like `val_protein=`, and excluding it from the numeric join; pinned by `test_named_eval_sources_reach_the_log_without_a_format_error`. Stage IV also brought the first metric names containing `/` -- `stage4/aa_pre` on a row, `binder_pinder/loss` on the mean -- and a `/` is a word boundary, so `plot_training_metrics.py` read a bare `aa_pre`, and a bare `loss`, off every validation line and overwrote that step's real training point. Metric names are now flattened for the log only (`val_stage4_aa_pre`, `val_binder_pinder_loss`); the returned dict and its JSON keep their slashes. The documented GPU smoke cannot catch this class of failure at all -- `smoke_stage4_fampnn.py` sets `eval_interval=0`. So prove the eval path on a short job (`MAX_STEPS=4 EVAL_INTERVAL=2 EVAL_SAMPLES=4 ITERS_TO_ACCUMULATE=1`), and keep `CHECKPOINT_INTERVAL` below `EVAL_INTERVAL` so a future validation failure costs one interval instead of the whole slot.
...
The HAI rerun originally returned 574 passed / 1 failed: the backbone-mapping fixture used `hash(atom_name) % 7`, allowing N and C to coincide under some `PYTHONHASHSEED` values. The fixture now uses fixed, noncollinear backbone geometry; the production degeneracy guard is unchanged. The six mapping tests passed with seed 1, and the formerly failing frame test also passed with seeds 4, 6 and 10. Another 22 launcher, smoke-selection, Stage IV contract and strict-input tests passed. These are focused checks; the full suite has not been rerun after this repair.

HAI job 113676 ran the gate on the Stage III donor at `stage3_binder_coevolution/111408/checkpoints/step6000.pt`: donor load `unexpected=0`, all loss components finite, both feedback-gradient routes nonzero into the real packer (`aa_to_sc` 1.9e4, `bb_to_sc` 7.8e5), the sampled frozen parameter bit-identical after the optimizer step, save/resume `missing=0, unexpected=0`, and a three-round generation exported to mmCIF. On that one 48-token complex at step 1, `stage4/recovery_pre` was 2.8% and `stage4/recovery_revision` 2.4% -- an initialization reading on a single item, not a measurement. `stage4/sc_aux` was 0.0 on that item. That is item-specific, not structural: `DesignSourceDataset` captures native binder side chains before strict re-featurization and restores them as loss *targets* while keeping them out of the structural input, and in job 113677 the same term reads 1.9-6.2 across accumulation micro-batches. A single smallest-by-token complex with no resolved side chains under the design mask zeroes the masked loss on its own.

That GPU run covered the original feedback, optimizer, resume and export checks, but did not exercise native-side-chain auxiliary supervision. The smoke now sorts candidates by token count and PINDER ID, selects an item with observed binder side-chain targets, records observed atom/residue counts, and requires finite nonzero `sc_aux_to_sc` gradients as well as both feedback routes. Local strict featurization selected `1aw8__A1_P0A790--1aw8__C1_P0A790` with 101 observed side-chain atoms across 22 binder residues. The expanded real-model GPU gate remains to be run. The earlier CPU attempt exposed and fixed a pre-attention feedback hook issue and was stopped after discovering an unintended eight-sample setting; it is not part of the GPU result.

Still open on the training side. HAI job **113677** is the first IV-A run (crop 384, 23:50 slot) and was still in progress when this was written -- no Stage IV training result exists. Its step-50 readings, for orientation only: `stage4/aa_pre` 2.61-3.05 against ln 20 = 3.00, `stage4/recovery_pre` 6.9-22%, `loss_bb` 0 as IV-A intends, and one log line per accumulation micro-batch rather than per step. The periodic `binder_pinder` / `monomer_retention` validation path has not run on a real model at all: it first fires at step 2000, roughly seven hours in, and job **113680** was queued to exercise it in four steps instead.
