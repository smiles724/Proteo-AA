...
def parse_args(argv=None):
    ...
    p.add_argument(
        "--input-json", required=True, help="PXDesign inference input (JSON or YAML)"
    )
...
def main(argv=None):
    ...
    logger.info(
        "loading FaMPNN designer (variant %s, %d unmasking steps, T=%.2f)",
        args.fampnn_weights,
        args.seq_steps,
        args.temperature,
    )
    designer = FaMPNNFullAtomDesigner(
        ...
        repack_last=not args.no_repack_last,
    ...
    for batch in backbone.generate(seed=args.seed):
        ...
        fixed = (
            torch.zeros_like(known) if args.redesign_target else (known & ~design_tokens)
        )
        aatype = atom37.aatype_from_sequence(batch.native_sequence, allow_unknown=True)
        logger.info(
            "%s: %d samples, L=%d, %d design token(s), %d position(s) held fixed",
            batch.sample_name,
            batch.num_samples,
            batch.length,
            int(design_tokens.sum()),
            int(fixed.sum()),
        )
        ...
        for index, design in enumerate(result["designs"]):
            ...
            logger.info(
                "  sample %d: designed %d residues, mean psce %.3f -> %s",
                index,
                entry["n_designed"],
                entry["mean_psce"],
                path.name,
    ...
    record = dict(
        designs=manifest,
        provenance=dict(
            pipeline="pxdesign->fampnn(seq_design)",
