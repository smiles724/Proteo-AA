...
重做**，有两种方式 torsion angle 和 cartesian coordinate，我进行了 torsion angle 的实验，是用 APM 的 one-step torsion packer。

后半周用 APM 自己的数据和目标函数训了四个序列条件。**结论：两条序列条件
...
| 阶段 | 产出 |
|---|---|
| **定位** | designability 全量 0/3280；血缘定位到"两个失效叠加" |
| **机制** | 找到侧链读出层的 Jensen 收缩；决定改成扭转角输出 |
| **重做** | 移植 APM `SideChainModel`，证明等价；PXDesign 数据上跑首轮 2×2 |
| **对齐 + 消融** | 换成 APM 自己的数据/损失/sampler；a_token 特征桥；四臂完成并评测 |
...
**② 真正的瓶颈是数据量，不是条件信息 —— 已用实验证实（09-17）。**

原观察：同架构、同损失、同 sampler，我们 1.611 vs 官方 1.347，差 0.26 Å，
是序列条件那一项（0.018）的 14 倍。

**验证**：接上 APM 自己的 `use_AFDB` + `use_SWISSPROT`（18,373 → 186,695 条，
job 118011，26 epoch / 22h），449 条统一估计器：

| 臂 | 数据 | symmetry_rmsd | vs 官方 1.3469 |
|---|---|---|---|
| `none` | 18,373 | 1.6109 | +0.264 |
| `none` | **186,695** | **1.3823** | **+0.035** |
| `plm` | 18,373 | 1.6119 | +0.265 |
| `plm` | **186,695** | **1.3608** | **+0.014** |

**数据量解释了 87–95% 的差距。** chi 指标同步追上（chi1 0.797 → 0.849，
官方 0.856；rotamer 0.605 → 0.674，官方 0.685）。

顺带一个反转：全数据下 `plm` 反超 `none` 0.021 Å 并在所有 chi 指标上更好，
而 monomer-only 时它是持平（+0.001）。**但三者现在挤在 0.035 Å 内，
单 seed 在这个分辨率下不足以判断**，记为现象不作结论。

训练曲线在 epoch 26（22h 墙钟）时仍在小幅下降，**没有跑到收敛**。
