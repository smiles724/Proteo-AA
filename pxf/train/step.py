...
REQUIRED_KEYS = ("x", "aatype", "seq_mask", "missing_atom_mask",
...
class StepOutput:
    ...
    def scalars(self):
        ...
        out = {"loss": float(self.total), "loss_main": float(self.mlm + self.diffusion),
               "loss_mlm": float(self.mlm), "loss_diffusion": float(self.diffusion)}
...
def encoder_inputs(model, batch, mar_out):
    ...
    atom_mask[..., rc.non_bb_idxs] = (atom_mask[..., rc.non_bb_idxs]
...
def sidechain_targets(model, batch):
    ...
    atom_mask_bb = ((1 - batch["missing_atom_mask"][..., rc.bb_idxs])
                    * batch["seq_mask"].unsqueeze(-1))
...
def diffusion_loss(model, batch, mpnn_feature_dict, *, multiplier=None,
                   self_cond_p=None, generator=None):
    ...
    multiplier = int(getattr(module.cfg, "training_batch_size_mult", 1)
                     if multiplier is None else multiplier)
    self_cond_p = (float(getattr(module.cfg, "self_cond_p", 0.0))
                   if self_cond_p is None else self_cond_p)
    ...
    x1_pred, _ = denoiser(xt, aatype_rep, t, h_V_rep, seq_mask_rep,
                          x_scn_self_cond=self_cond)
...
def _rollout_packed_sidechains(model, batch, mpnn_feature_dict, aatype):
    ...
    _, aux = module.sidechain_diffusion(
        mpnn_feature_dict, aatype, seq_mask=batch["seq_mask"],
        residue_index=batch["residue_index"], chain_index=batch["chain_index"],
        aux_inputs={"scd": scd_inputs}, is_sampling=True)
    return aux
...
def confidence_loss(model, batch, mpnn_feature_dict, aatype):
    ...
    detached = {k: (v.detach() if torch.is_tensor(v) else v)
                for k, v in mpnn_feature_dict.items()}
    ...
    packed_local, _ = transform_sidechain_frame(
        packed_global, batch["x"][..., rc.bb_idxs, :], atom_mask, atom_mask_bb,
        to_local=True)
    packed_local = packed_local.detach()

    psce_logits, _ = module.confidence_module(
        packed_local, detached, aatype, batch["seq_mask"],
        batch["residue_index"], batch["chain_index"])
    spec = loss_fns.psce_bin_spec(module.confidence_module)
    return loss_fns.confidence_loss(psce_logits, packed_local, x1_local, atom_mask,
                                    bin_spec=spec)


def training_forward(model, batch, *, train_confidence=None, generator=None,
                     multiplier=None, self_cond_p=None):
    ...
    seq_logits, mpnn_feature_dict = model.denoiser.seq_design_module(
        mar_out["x_noised"], mar_out["aatype_noised"], batch["seq_mask"],
        atom_mask_noised, batch["residue_index"], batch["chain_index"])
    ...
    loss_diff, diff_stats = diffusion_loss(
        model, batch, mpnn_feature_dict, multiplier=multiplier,
    ...
    if train_confidence is None:
        train_confidence = (module.use_confidence_module
    ...
    with torch.no_grad():
        ...
        stats["keep_fraction"] = (mar_out["seq_mlm_mask"].sum()
                                  / batch["seq_mask"].sum().clamp_min(1))
    return StepOutput(total=loss_fns.total_loss(loss_mlm, loss_diff, loss_conf),
                      mlm=loss_mlm, diffusion=loss_diff, confidence=loss_conf,
                      stats=stats)
