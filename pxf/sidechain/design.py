...
class DesignResult:
    ...
    seq_probs: torch.Tensor | None = None
    fixed_mask: torch.Tensor | None = None  # [L] True where identity was held

    def designed_sequence(self):
        if self.fixed_mask is None:
            return self.sequence
        return "".join(
            a for a, fixed in zip(self.sequence, self.fixed_mask.tolist()) if not fixed
        )
...
class FaMPNNFullAtomDesigner(FaMPNNSideChainPacker):
    ...
    def __init__(
        ...
        *,
        ...
        repack_last=DEFAULT_REPACK_LAST,
        seq_timestep_mode="linear",
        **kwargs,
    ):
        ...
        self.identity.update(
            ...
            psce_threshold=self.psce_threshold,
            repack_last=self.repack_last,
    ...
    def _seq_timesteps(self, batch):
        ...
        steps = sampling_utils.get_timesteps_from_schedule(
            mode=self.seq_timestep_mode, num_steps=self.seq_steps, t_start=0.0, t_end=1.0
        )
    ...
    def design(
        ...
        *,
        ...
        fixed = (
            torch.zeros(batch, length, dtype=torch.long, device=self.device)
            if fixed_sequence_mask is None
            else self._as_batched(fixed_sequence_mask, "fixed_sequence_mask", batch, length)
            .long()
            .to(self.device)
        )
        context = (
            torch.zeros_like(fixed)
            if sidechain_context_mask is None
            else self._as_batched(
                sidechain_context_mask, "sidechain_context_mask", batch, length
            )
            .long()
            .to(self.device)
        )
        ...
        for start in range(0, batch, chunk):
            ...
            missing = self.missing_atom_mask(
                aatype[piece].clamp_max(atom37.UNKNOWN_AA_INDEX), atom_mask[piece]
            )
            designed, aatype_new, aux = self.model.sample(
                ...
                repack_last=self.repack_last,
                psce_threshold=self.psce_threshold,
                aatype_override_mask=fixed[piece],
                scn_override_mask=context[piece],
        ...
        results = [
            DesignResult(
                coords_af2=designed[i],
                atom_mask_af2=self._output_atom_mask(
                    aatype_new[i][None], atom_mask[i][None]
                )[0],
                aatype=aatype_new[i],
    ...
    def _normalize_for_design(
        self, coords, aatype, atom_mask, seq_mask, residue_index, chain_index
    ):
