...
| **Stage III — teacher-forced co-evolution** | implemented; fresh smoke / full training pending | `S_φ` reads **GT backbone coordinates/frames + GT atom set**, while learned `h/a/q` and type-logit features come from `B_pre`; its feedback drives `B_post(x_hat_0)` with fixed `refinement_sigma=2.0`. |
| **Stage IV — inference-matched co-evolution** | implemented, training pending | `S_φ` switches to the **predicted backbone frame and predicted residue atom set**; type-matched residues receive coordinate supervision and mismatches use compatibility/physical routing, making `post_aa` safe to supervise. |
