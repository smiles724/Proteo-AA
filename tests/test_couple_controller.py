...
from pxf.couple.controller import CoupledDenoiser, GradientPolicy, PHASES, Topology
...
def case():
    ...
    topology = Topology(
        atom_names=names, atom_to_token_idx=tokens, num_tokens=length,
...
def build(fampnn, case, phase="joint", **kwargs):
    ...
    controller = CoupledDenoiser(backbone, fampnn, adapters, phase=phase,
...
def test_the_cycle_runs_both_directions(fampnn, case):
    ...
    torch.manual_seed(0)
    out = controller.forward(case["topology"], case["flat"], torch.tensor([1.0]),
                             case["aatype"])
...
def test_phase_zero_equivalence_is_exact(fampnn, case):
    ...
    torch.manual_seed(0)
    coupled = controller.forward(case["topology"], case["flat"], torch.tensor([1.0]),
                                 case["aatype"])
    inputs = coupled.aux["inputs"]
    _, _, features = iface.encode(
        fampnn, coupled.bb0_dense, inputs.aatype, seq_mask=inputs.seq_mask,
    ...
    reference, _ = iface.pack_from_features(
        fampnn, features, inputs.aatype, seq_mask=inputs.seq_mask,
        residue_index=inputs.residue_index, chain_index=inputs.chain_index,
        num_steps=PACK_STEPS)
    torch.manual_seed(0)
    again = controller.forward(case["topology"], case["flat"], torch.tensor([1.0]),
                               case["aatype"])
...
def test_bb_to_sc_gradients_reach_only_a_bs(fampnn, case):
    ...
    torch.manual_seed(0)
    out = controller.forward(case["topology"], case["flat"], torch.tensor([1.0]),
                             case["aatype"], run_feedback=False)
    out.sidechains.sum().backward()
    into_bs = sum(float(p.grad.abs().sum()) for p in adapters.bb_to_sc.parameters()
                  if p.grad is not None)
    into_sb = sum(float(p.grad.abs().sum()) for p in adapters.sc_to_bb.parameters()
                  if p.grad is not None)
...
def test_a_backbone_without_token_features_is_refused(fampnn, case):
    ...
    controller = CoupledDenoiser(lambda x, s, feedback=None: (x, None), fampnn,
                                 adapters, pack_steps=PACK_STEPS)
    with pytest.raises(ValueError, match="no token features"):
        controller.forward(case["topology"], case["flat"], torch.tensor([1.0]),
                           case["aatype"])
