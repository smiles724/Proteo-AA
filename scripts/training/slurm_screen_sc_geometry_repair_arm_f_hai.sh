#!/usr/bin/env bash
#SBATCH --job-name=sc-repair-f-screen
#SBATCH --partition=yejin
#SBATCH --account=yejin
#SBATCH --cpus-per-task=2
#SBATCH --mem=8G
#SBATCH --time=00:30:00
#SBATCH --output=logs/training/sc-repair-f-screen-%j.out
#SBATCH --error=logs/training/sc-repair-f-screen-%j.err
set -euo pipefail

REPO=${PROTEOAA_REPO:-/hai/users/y/f/yfsun/Proteo-AA-sc-adaptation-phases}
PYTHON_BIN=${PYTHON_BIN:-/hai/users/y/f/yfsun/miniconda3/envs/ml/bin/python}
RUN_ROOT=${SC_REPAIR_V2_RUN_ROOT:?Set SC_REPAIR_V2_RUN_ROOT to the directory containing arm_F}
PREREGISTRATION=${SC_REPAIR_V2_PREREGISTRATION:-$REPO/runs/sc_geometry_repair/sweep_v2_arm_f/preregistered_gate.json}
ARM_B=${SC_REPAIR_ARM_B:-/hai/scratch/yfsun/proteo_aa_runs/sc_geometry_repair/arms_115668/arm_B}
DONOR_BASELINE=${SC_REPAIR_DONOR_BASELINE:-/hai/scratch/yfsun/proteo_aa_runs/sc_geometry_repair/donor_baseline/donor_baseline.json}
cd "$REPO"
exec "$PYTHON_BIN" scripts/utilities/select_sc_geometry_repair_arm_f.py \
  --preregistration "$PREREGISTRATION" --arm-b "$ARM_B" \
  --arm-f "$RUN_ROOT/arm_F" --donor-baseline "$DONOR_BASELINE" \
  --output "$RUN_ROOT/screen.json"
