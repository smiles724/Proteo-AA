...
* `template_init_local` — Overleaf paragraph 221 + the 0714 appendix
  ("Residue-Specific Side-Chain Template Construction"), the
  "template-anchored leakage-free initialization":

      y_{T,ij} = mu_ideal[a_i, chi_i, j] + sigma_T * eps_ij,   eps ~ N(0, I)
      x_{T,ij} = F_hat_i y_{T,ij}

  mu_ideal is the residue's ideal covalent geometry posed at torsions chi_i, where
  chi_i is drawn from a backbone-dependent rotamer library conditioned on the
  PREDICTED backbone dihedrals (phi_i, psi_i). It is ANISOTROPIC, so once rotated by
  F_hat the global initialization encodes the backbone orientation — which is the
  entire point of the paragraph. It depends on residue TYPE, the atom mask, and the
...
def _ideal_template(
    ...
    `generator`, `phi` and `psi` are forwarded so the backbone-dependent rotamer provider
    (0714 appendix Step 2) works without any caller knowing about rotamers. `backbone` is
    legacy and unused by the shipped providers -- see templates.py for why a per-residue
    local-frame backbone cannot express phi/psi.

    Providers written before phi/psi existed take only (type_idx, generator, backbone), so
    fall back to that signature rather than breaking a registered provider.
    """
    from pxdesign_train.sidechain.templates import get_ideal_template_provider

    provider = get_ideal_template_provider()
    try:
        return provider(
            type_idx, generator=generator, backbone=backbone, phi=phi, psi=psi
        )
    except TypeError:
        return provider(type_idx, generator=generator, backbone=backbone)
...
def template_init_local(
    ...
    """Overleaf par.221 + 0714 appendix: y_T = mu_ideal[a, chi(a, phi, psi), j] + sigma_T * eps.

    Template-anchored, leakage-free side-chain initialization in the residue-local
    frame. Conditions on residue type, the atom mask, and the PREDICTED backbone's
    dihedrals -- all inference-available. It never sees ground-truth side-chain
    coordinates.
    ...
    Args:
        ...
        phi, psi: [...] float radians from the PREDICTED backbone
            ...
            `type_idx`. Omitting them costs the backbone conditioning: the provider
            falls back to the marginal rotamer distribution.
    ...
    for name, ang in (("phi", phi), ("psi", psi)):
        if ang is not None:
            assert ang.shape == type_idx.shape, (
                f"{name} {tuple(ang.shape)} must match type_idx {tuple(type_idx.shape)}"
            )
    # Out-of-place: type_idx is often an expand()ed view (per-sigma tiling).
    safe_idx = type_idx.long().clamp(0, len(STD_AA_3) - 1)
    mu, tmask = _ideal_template(
        safe_idx, generator=generator, backbone=backbone, phi=phi, psi=psi
    )
