...
`evaluate()` runs *before* `save_checkpoint()` in the training loop, so a failure in the new binder/monomer validation path at the first eval step discards everything before it with no checkpoint written. That is not hypothetical: the named Stage IV loaders stamp a string `source` on every per-protein row, the per-protein log line formatted every field with `:.4g`, and `f"{'binder_pinder':.4g}"` raises `ValueError: Unknown format code 'g' for object of type 'str'`. Fixed by emitting `val_source=` explicitly, like `val_protein=`, and excluding it from the numeric join; pinned by `test_named_eval_sources_reach_the_log_without_a_format_error`. The documented GPU smoke cannot catch this class of failure at all -- `smoke_stage4_fampnn.py` sets `eval_interval=0`. So prove the eval path on a short job (`MAX_STEPS=4 EVAL_INTERVAL=2 EVAL_SAMPLES=4 ITERS_TO_ACCUMULATE=1`), and keep `CHECKPOINT_INTERVAL` below `EVAL_INTERVAL` so a future validation failure costs one interval instead of the whole slot.
