...
def parse_args(argv=None):
    ...
    parser.add_argument("--pxdesign-checkpoint-dir", required=True,
    ...
    parser.add_argument("--num-steps", type=int, default=None, help="FaMPNN diffusion steps")
    parser.add_argument("--step-scale", type=float, default=None)
    parser.add_argument("--keep-sidechain-context", action="store_true",
...
def main(argv=None):
    ...
    supplied = args.sequence or (read_fasta(args.sequence_fasta) if args.sequence_fasta else None)

    logger.info("loading side-chain module (FaMPNN %s, packing mode)", args.weights)
    sidechain = FaMPNNSideChainPacker(
        args.fampnn_checkpoint, variant=args.weights, num_steps=args.num_steps,
        step_scale=args.step_scale, strict_sources=strict)
    sidechain = sidechain.to(select_device(args.device))
    logger.info("side-chain module on %s (designs_sequence=%s)", sidechain.device,
    ...
    backbone = PXDesignBackbone(
        ...
        dump_dir=out / "pxdesign", model_name=args.model_name, n_sample=args.n_sample,
        n_step=args.n_step, use_msa=args.use_msa, dtype=args.dtype,
        download_cache=not args.no_download_cache, strict_sources=strict)
    logger.info("backbone module on %s; %d input target(s)", backbone.device, len(backbone))

    pipeline = FullAtomPipeline(
        backbone, sidechain, sequence=supplied, batch_size=args.batch_size,
    ...
    for batch in backbone.generate(seed=args.seed):
        logger.info("%s: %d backbone sample(s), %d residues, %d design token(s)",
        ...
        for result in pipeline.run_batch(batch):
            ...
            manifest.append(dict(
                sample_name=result.sample_name, sample_index=result.sample_index,
                pdb=str(path.relative_to(out)), length=result.length,
                sequence=result.sequence, designed_sequence=result.designed_sequence(),
                backbone_shift_angstrom=result.backbone_shift, **result.metrics))
            count += 1
            logger.info("  sample %d: mean psce %.3f -> %s", result.sample_index,
                        result.metrics["mean_psce"], path.name)
