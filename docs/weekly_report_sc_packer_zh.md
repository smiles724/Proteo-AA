...
默认已改成 400 步。**这不推翻①，但②的口径不干净，必须标注**：②的 60% 用的是
旧 probe（不检查链断裂），而我们自己的 Stage II/III checkpoint **从未在
400 步 + 修正 probe 下重测过**。所以"我们 60% vs 官方 0%"从来不是一次同口径比较。

**站得住的只有两条**：(a) 0/3280 的 designability；(b) 失效**方向相反**——
我们塌缩到 CA 间距 2.75 Å，官方在 20 步下是张开到 7.66 Å。
