...
def main(argv=None):
    ...
    if args.pdb_dir:
        ...
        paths = [Path(p) for p in natsorted(str(p) for p in Path(args.pdb_dir).glob("*.pdb"))]
    else:
    ...
    packer = FaMPNNSideChainPacker(
        args.checkpoint, variant=args.weights, num_steps=args.num_steps,
        step_scale=args.step_scale, strict_sources=not args.allow_unpinned_sources)
    ...
    for path in paths:
        ...
        packed = packer(coords_af2=coords[None], aatype=single["aatype"][None],
                        atom_mask=atom_mask[None],
                        residue_index=single["residue_index"][None],
                        chain_index=single["chain_index"][None],
                        scn_context_mask=(torch.ones(1, length) if args.keep_sidechain_context
                                          else torch.zeros(1, length)),
                        batch_size=args.batch_size)
        from pxf.pipeline import PackedDesign
        result = PackedDesign(
            sample_name=path.stem, sample_index=0, sequence=sequence,
            ...
            chain_index=single["chain_index"].long(), psce=packed["psce"][0].cpu(),
        ...
        manifest.append(dict(name=path.stem, source=str(path), length=length,
                             sequence=sequence, pdb=str(written.relative_to(out)),
                             mean_psce=result.metrics["mean_psce"],
                             backbone_shift_angstrom=result.backbone_shift))
        logger.info("%s: L=%d mean psce %.3f backbone shift %.4f A -> %s", path.stem,
                    length, result.metrics["mean_psce"], result.backbone_shift, written.name)

    (out / "manifest.json").write_text(json.dumps(
        dict(packed=manifest, provenance=dict(sidechain=packer.identity),
