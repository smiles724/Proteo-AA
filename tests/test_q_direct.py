...
def test_config_default_ships_the_q_channel_zero_initialised():
    """q_direct ships ON as part of the full bidirectional default.

    That is only safe because the fusion is zero-initialised: at step 0
    ...
    zero-init assertion is load-bearing, not cosmetic.

    The a-level channel is split across two injection points -- `a_direct_pre`
    (before the global cross-residue attention, the default) and `a_direct`
    (after it) -- so `a_direct` being False is not "the a channel is off".
    """
