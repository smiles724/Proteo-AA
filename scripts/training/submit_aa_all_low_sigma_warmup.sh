#!/bin/bash
# Submit one strong low-sigma diagnostic:
#   PINDER-only, AA-head-only, and all 8 diffusion samples fixed at 0.04/0.4.
#
# This intentionally uses uniform AA loss weighting.  Since every sample is
# already low sigma, an additional sigma weighting would confound the control.
...
CONTROL_ROOT="${CONTROL_ROOT:-/hai/scratch/shenjm/proteo_aa_runs/aa_all_low_sigma_warmup}"
...
export RUNS_ROOT="${CONTROL_ROOT}"

job_raw="$({
  sbatch --parsable --job-name="aa-alllow-c${CROP_SIZE}" "${TRAIN_SCRIPT}" \
...
echo "all-low-sigma train: ${job_id}"
echo "checkpoint         : ${CONTROL_ROOT}/stage3_binder_coevolution/${job_id}/checkpoints/step${TRAIN_STEPS}.pt"
