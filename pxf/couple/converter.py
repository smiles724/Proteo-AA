...
class CoupledInputs:
    ...
    def fampnn_kwargs(self):
        ...
        return dict(coords_af2=self.coords_af2, atom_mask=self.atom_mask,
                    seq_mask=self.seq_mask, residue_index=self.residue_index,
...
class PXFaRepresentationConverter:
    ...
    def px_backbone_to_fampnn(self, coords, atom_names, atom_to_token_idx, num_tokens, *,
                              res_names=None, residue_index=None, chain_index=None,
                              aatype=None, strict=False):
        ...
        if res_names is not None:
            design = bridge.design_mask_from_res_names(res_names, atom_to_token_idx, num_tokens)
            sequence, known = bridge.native_sequence(res_names, atom_to_token_idx, num_tokens)
        ...
        expand = lambda t: t.reshape(1, num_tokens).expand(batch, num_tokens).contiguous()
        seq_mask = torch.ones(batch, num_tokens, device=dense.device)
        return CoupledInputs(
            coords_af2=dense, atom_mask=mask, aatype=expand(aatype),
            seq_mask=seq_mask,
            missing_atom_mask=self.missing_atom_mask(expand(aatype), mask),
            residue_index=expand(residue_index), chain_index=expand(chain_index),
            design_mask=design, sequence_known=known, num_tokens=int(num_tokens),
    ...
    def px_residue_mask_to_fampnn(self, mask, num_tokens, *, batch=1, dtype=torch.float32):
        ...
        return tensor.reshape(1, num_tokens).expand(batch, num_tokens).to(dtype).contiguous()

    def map_chain_and_residue_indices(self, num_tokens, *, residue_index=None,
                                      chain_index=None):
        ...
        if chain_index is None:
            ...
            order = {value: position for position, value in
                     enumerate(dict.fromkeys(raw.tolist()))}
    ...
    def missing_atom_mask(self, aatype, atom_mask):
        ...
        exists = get_rc_tensor(self.rc.STANDARD_ATOM_MASK_WITH_X,
    ...
    def fampnn_sidechains_to_px(self, sidechains, coords_af2=None):
        ...
        if block.shape[-2] != len(self.sidechain_slots):
            raise ValueError(f"expected {len(self.sidechain_slots)} side-chain atoms, "
                             f"got {block.shape[-2]}")
    ...
    def scatter_to_px_atoms(self, coords_af2, atom_names, atom_to_token_idx):
        ...
        slots = torch.tensor([lookup.get(str(name), -1) for name in atom_names],
                             dtype=torch.long)
    ...
    def identity(self):
        return dict(component="PXFaRepresentationConverter",
