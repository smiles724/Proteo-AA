...
REPORT = {
    ...
    "rmsd": ("symmetry_rmsd",),
    "angle correctness": ("chi_recovery_20deg", "chi_recovery_40deg",
                          "chi1_accuracy_20deg", "chi1_chi2_accuracy_20deg",
                          "rotamer_recovery"),
    "covalent failures": ("bad_bond_fraction", "bond_mae",
                          "rotamer_outlier_fraction_40deg", "completeness"),
}
...
def parse_args(argv=None):
    ...
    parser.add_argument("--pdb-key-list", default=None,
                        help="optional file of stems to restrict to")
    ...
    parser.add_argument("--num-steps", type=int, default=None,
                        help="FaMPNN side-chain diffusion steps")
    parser.add_argument("--samples", type=int, default=1,
                        help="packings per target; packing is a sampler")
...
def main(argv=None):
    ...
    if args.pdb_key_list:
        stems = [s.strip() for s in Path(args.pdb_key_list).read_text().split() if s.strip()]
    ...
    logger.info("metrics from %s @ %s (not reimplemented)", canonical.root, canonical.revision[:10])

    packer = FaMPNNSideChainPacker(args.checkpoint, variant=args.weights,
                                   num_steps=args.num_steps,
                                   strict_sources=not args.allow_unpinned_sources)
    packer = packer.to(select_device(args.device))
    logger.info("FaMPNN %s on %s; %d monomer target(s) from %s",
                packer.variant, packer.device, len(paths), label)
    ...
    for index, path in enumerate(paths):
        ...
        for sample in range(args.samples):
            ...
            packed = packer(coords_af2=coords[None], aatype=aatype[None],
                            atom_mask=given[None],
                            residue_index=single["residue_index"][None],
                            chain_index=single["chain_index"][None], seed=seed)
            counts, summary = score(
                packed["coords_af2"][0].cpu(), packed["atom_mask_af2"][0].cpu(),
                native, native_mask, aatype,
                canonical=canonical, residue_mask=canonical_res)
            per_target_counts.append(counts)
            rows.append(dict(target=path.stem, sample=sample, length=length,
        ...
        last = rows[-1]
        logger.info("%-12s L=%-4d rmsd=%.3f lddt_sc=%.3f lddt_env=%.3f chi20=%.3f bad_bond=%.4f",
                    path.stem, length, last["symmetry_rmsd"], last["lddt_sc_sc"],
                    last["lddt_sc_env"], last["chi_recovery_20deg"], last["bad_bond_fraction"])
    ...
    record = dict(
        label=label, dataset=str(pdb_dir), n_targets=len(paths), n_scored=len(rows),
        samples_per_target=args.samples, seed=args.seed, skipped=skipped,
        elapsed_seconds=round(elapsed, 1), summary=summary, per_target=rows,
        provenance=dict(sidechain=packer.identity, metrics=canonical.record(),
