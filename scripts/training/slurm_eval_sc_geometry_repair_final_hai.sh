#!/usr/bin/env bash
#SBATCH --job-name=sc-repair-final
...
#SBATCH --output=logs/training/sc-repair-final-%j.out
#SBATCH --error=logs/training/sc-repair-final-%j.err
set -euo pipefail
...
exec "$PYTHON_BIN" scripts/utilities/evaluate_sc_geometry_repair_final.py \
  --acceptance "$RUN_ROOT/acceptance.json" --output "$OUTPUT" --device cuda
