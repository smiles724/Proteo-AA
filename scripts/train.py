...
def main(argv=None):
    ...
    from pxf.train.trainer import OptimSettings, Trainer, TrainSettings

    cfg = load_config(args.config)
    data_cfg, train_cfg, optim_cfg = (
        dict(cfg.get(k, {})) for k in ("data", "train", "optim")
    )
    for key, value in (
    ...
    source = (
        ...
        if Path(args.init_weights).exists()
