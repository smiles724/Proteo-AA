#!/bin/bash
#SBATCH --job-name=proteo-aa-sc-template
...
RUN_ROOT="${RUN_ROOT:-/hai/scratch/yfsun/proteo_aa_runs/protenix_monomer_sidechain_warmup/template_baseline_491}"
TRAIN_LOG="${TRAIN_LOG:-${REPO_ROOT}/logs/training/sidechain_warmup/proteo-aa-sc-warm-97706.out}"
FIGURE="${FIGURE:-${REPO_ROOT}/runs/figures/sidechain_warmup_97706_with_template_baseline.png}"
METRICS_CSV="${METRICS_CSV:-${REPO_ROOT}/runs/metrics/sidechain_warmup_97706_with_template_baseline.csv}"
RANKING_CSV="${RANKING_CSV:-${REPO_ROOT}/runs/metrics/sidechain_warmup_97706_checkpoint_ranking_with_template_baseline.csv}"
...
if [[ ! -f "${TRAIN_LOG}" ]]; then
  echo "ERROR: training log does not exist: ${TRAIN_LOG}" >&2
  exit 2
fi

mkdir -p "${REPO_ROOT}/logs" "${RUN_ROOT}" "$(dirname "${FIGURE}")" "$(dirname "${METRICS_CSV}")"
cd "${REPO_ROOT}"
...
"${PYTHON_BIN}" scripts/evaluation/eval_sidechain_template_baseline.py \
  --data-root "${DATA_ROOT}" \
...
TEMPLATE_BASELINE="$("${PYTHON_BIN}" -c 'import json,sys; print(json.load(open(sys.argv[1]))["sample_mean_mse"])' "${RUN_ROOT}/template_baseline.json")"

"${PYTHON_BIN}" scripts/plotting/plot_sidechain_warmup_report.py \
  --log "${TRAIN_LOG}" \
  --out "${FIGURE}" \
  --metrics-csv "${METRICS_CSV}" \
  --ranking-csv "${RANKING_CSV}" \
  --template-baseline "${TEMPLATE_BASELINE}"

echo "template_baseline=${TEMPLATE_BASELINE}"
echo "wrote_figure=${FIGURE}"
