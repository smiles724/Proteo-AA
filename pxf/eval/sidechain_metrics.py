...
def score(pred37, pred_mask37, native37, native_mask37, aatype, *,
          canonical=None, residue_mask=None):
    ...
    rotation, translation = canonical.frames.build_frame(
        ncac[:, 0], ncac[:, 1], ncac[:, 2])
    ...
    counts = canonical.packing_metrics(
        aatype, pred_local, bb_local, generated, scored,
        target=native_local, observed=observed, target_bb=bb_local)
    ...
    lddt = canonical.sidechain_lddt(
        torch.nan_to_num(pred_sc), torch.nan_to_num(native_sc), both,
        bb_coords=native37[:, list(BACKBONE), :], bb_mask=bb_mask)

    counts = {key: value.detach().cpu() if torch.is_tensor(value) else value
              for key, value in counts.items()}
    for key in LDDT_KEYS:
        counts[f"{key}_pairs"] = torch.tensor(float(lddt[f"n_pairs_{key[5:]}"]))
        counts[f"{key}_sum"] = torch.tensor(float(lddt[key]) * float(lddt[f"n_pairs_{key[5:]}"]))
    summary = {key: float(value) for key, value in
               canonical.summarize_metrics({k: v for k, v in counts.items()
                                            if torch.is_tensor(v)}).items()}
...
def aggregate(per_target, *, canonical=None):
    ...
    summary = {key: float(value) for key, value in
               canonical.summarize_metrics(totals).items()}
