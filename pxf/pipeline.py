...
class PackedDesign:
    ...
    backbone_shift: float | None = None
    ...
    def designed_sequence(self):
        ...
        return "".join(
            a for a, keep in zip(self.sequence, self.design_mask.tolist()) if keep
        )
...
class FullAtomPipeline:
    ...
    def __init__(
        self, backbone, sidechain, *, sequence=None, scn_context="none", batch_size=None
    ):
        if scn_context not in SCN_CONTEXT_MODES:
            raise ValueError(
                f"Unknown scn_context {scn_context!r}; choose from {SCN_CONTEXT_MODES}"
            )
    ...
    def resolve_sequence(self, batch):
        ...
        try:
            ...
            raise ValueError(
                ...
                f"token(s) of {batch.length} residues, which have no native identity."
            ) from error
        ...
        for index in range(batch.num_samples):
            ...
            results.append(
                PackedDesign(
                    ...
                    design_mask=batch.design_mask,
                    residue_index=batch.residue_index,
                    chain_index=batch.chain_index,
                    psce=psce,
                    backbone_shift=packed["backbone_shift"],
                    metrics=dict(
                        num_designed=int(designed.sum()),
                        mean_psce=float(psce.mean()),
                        mean_psce_designed=float(psce[designed].mean())
                        if bool(designed.any())
                        else None,
