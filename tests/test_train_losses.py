...
def test_no_scored_positions_gives_a_finite_zero():
    ...
    loss, stats = L.sequence_mlm_loss(logits, torch.zeros(1, 3, dtype=torch.long),
...
def test_psce_bin_edges_and_saturation():
    ...
    cases = {0.0: 0, step - 1e-6: 0, step: 1, 2 * step: 2,
             L.PSCE_MAX_BIN: L.PSCE_NUM_BINS - 1, 99.0: L.PSCE_NUM_BINS - 1}
...
def test_confidence_loss_rewards_the_right_bin():
    ...
    good, stats = L.confidence_loss(logits, torch.zeros_like(delta), delta, torch.ones(1, 1, 1))
    wrong = torch.zeros_like(logits); wrong[..., 0] = 20.0
